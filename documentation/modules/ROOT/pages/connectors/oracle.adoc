...
:supports-skipping-unchanged-events:
...
ifdef::community[]
{prodname} can ingest change events from Oracle by using the native LogMiner database package, the https://docs.oracle.com/en/database/oracle/oracle-database/19/xstrm/introduction-to-xstream.html[XStream API],
or https://www.bersler.com/openlogreplicator/[OpenLogReplicator].
endif::community[]
ifdef::product[]
{prodname} can ingest change events from Oracle by using either the native LogMiner database package or the https://docs.oracle.com/en/database/oracle/oracle-database/19/xstrm/introduction-to-xstream.html[XStream API].
...
By default, the {prodname} Oracle connector uses the native Oracle LogMiner API to read and stream changes from database transaction logs.
You can configure {prodname} to use LogMiner in either of the following distinct modes:

Uncommitted changes mode:::

In uncommitted changes mode, the {prodname} Oracle connector receives a continuous stream of change events for both committed and in-flight uncommitted transactions.
Because Oracle interleaves operations from multiple transactions, the connector must buffer events until a corresponding commit or rollback is detected.
+
...
+
Uncommitted changes mode minimizes the load on the source database by delegating most processing to the connector.
Oracle LogMiner is responsible only for reading redo and archive logs from disk and streaming the raw changes.
+
Enable this mode by setting the xref:oracle-property-database-connection-adapter[`database.connection.adapter`] property to `logminer` (default).

Committed changes mode:::

In committed changes mode, the {prodname} Oracle connector instructs Oracle LogMiner to stream only committed changes.
Because the connector receives only committed events, it can immediately forward them to destination topics without maintaining an internal transaction buffer.
+
This mode requires significantly less connector memory, but shifts a much greater processing and memory burden to the source database.
For large transactions, this can easily exceed the database’s available PGA memory, potentially leading to `PGA_AGGREGATE_LIMIT` errors.
+
This mode is recommended only if you can guarantee that all transactions fit within the database’s PGA memory space.
Otherwise, use the uncommitted changes mode.
+
Enable this mode by setting the xref:oracle-property-database-connection-adapter[`database.connection.adapter`] property to `logminer_unbuffered`.

ifdef::community[]
OpenLogReplicator::

You can configure the {prodname} Oracle connector to use OpenLogReplicator, an open-source application that reads Oracle changes directly from the redo and archive logs with minimal impact on the database.
+
Enable OpenLogReplicator by setting the xref:oracle-property-database-connection-adapter[`database.connection.adapter`] property to `olr`.
For more information, see the xref:oracle-openlogreplicator-support[] section.
endif::community[]

Oracle XStream::

The {prodname} Oracle connector can also use Oracle XStream, a commercial component of Oracle GoldenGate.
+
...
If you use the {prodname} `DebeziumEngine`, the name is included in the connector JAR file.
...
// Type: procedure
// ModuleID: debezium-oracle-using-the-drop-transaction-signal-to-abandon-stale-transactions
// Title: Using the drop-transaction signal to abandon stale transactions
[id="oracle-drop-transaction-signal"]
=== Dropping a transaction

You can use {prodname} signaling to send a `drop-transaction` signal to the connector at run time to remove a transaction from the LogMiner buffer.
Removing a transaction can be helpful when a long-running transaction negatively affects connector performance.

A long-running transaction prevents the connector from processing subsequent change events.
Further, the stalled transaction can result in excessive memory buffering, affect log mining continuity, and result in SCN (system change number) drift, a condition in which the connector’s current SCN falls behind the range that is available in the redo logs.

During normal processing, {prodname} periodically flushes offsets to the Kafka Connect offset topic.
If a transaction runs longer than the defined offset flush interval, the connector is unable to commit an offset.
If the connector restarts after the last known offset is removed from the redo logs, it cannot find the resume point, which results in an error.

By submitting a `drop-transaction` signal, you can avoid errors by removing a problem transaction from the connector's internal LogMiner buffer before it results in negative consequences.

.Prerequisites
* The connector is configured to use one of the available link:{link-signalling}#sending-signals-to-a-debezium-connector[{prodname} signaling channels].
* You have access to the connector log file and can identify the ID of the transaction that you want to drop.

[IMPORTANT]
====
After you drop a transaction, the connector abandons uncommitted changes for that transaction and does not emit change events for them.
Use the `drop-transaction` signal only when you are certain that the transaction should not be processed.
====

ifdef::community[]
[NOTE]
====
As an alternative to manually dropping transactions, you can configure the xref:oracle-property-log-mining-window-max-ms[`log.mining.window.max.ms`] property to automatically advance the mining window when a long-running transaction exceeds a specified duration.
Unlike the `drop-transaction` signal, this approach continues to track the transaction and buffer its events, while preventing the mining window from growing indefinitely.
====
endif::community[]

.Procedure
* Send a signal that includes the standard `id`, `type`, and `data` properties of a {prodname} signal to your preferred signaling channel.
In the  `data` component of the signal, specify the ID of the transaction that you want to remove.
+
For example, to drop a transaction with ID `1a.2b.001f`, send the signal based on your chosen signal channel:
+
.Example SQL using the `source` signal channel
[source,sql,indent=0,subs="+attributes"]
----
INSERT INTO debezium.debezium_signal VALUES('drop-tx-1', 'drop-transaction', '{"transaction-id": "1a.2b.001f"}');
----
+
.Example message payload using the `kafka` signal channel
[source,json,indent=0,subs="+attributes"]
----
{
  "type": "drop-transaction",
  "data": {
    "transaction-id": "1a.2b.001f"
  }
}
----
...
[WARNING]
====
The `redo_log_catalog` mining strategy is deprecated, and is scheduled for removal in a future {prodname} release.
Consider migrating to the `hybrid` strategy, which provides equivalent schema tracking reliability with better performance and lower overhead.
====
...
[NOTE]
====
This feature is in an incubating state. This means that exact semantics, configuration options, and other details may change in future revisions based on feedback.
...
By default, the mining window slides block-by-block and is not pinned by transactions in the cache.
To pin the mining window to the oldest promoted (DML-bearing) transaction in the cache, set xref:oracle-property-log-mining-buffer-deferred-transaction-cache-pin[`log.mining.buffer.deferred.transaction.cache.pin`] to `true`.
This is similar to the non-deferred path, but only transactions that have emitted DML pin the window, so transactions that never progress beyond `START` do not widen the mining window.
As with the non-deferred path, if a pinned transaction's start SCN ages out of the available redo logs, the connector stops and a re-snapshot is required.
...
[IMPORTANT]
====
The {prodname} Infinispan buffer implementation requires that you configure a set of distinct cache configurations, each identified by a {prodname}-defined cache name.
You must configure caches for the following identifiers: `transactions`, `events`, `processed-transactions`, `rollbacks`, and `schema-changes`.

Each cache name is a hardcoded identifier that you specify in the XML of a {prodname} Oracle connector configuration property in the `log.mining.buffer.infinispan.cache` namespace.
For example, the property `log.mining.buffer.infinispan.cache.transactions` contains the XML configuration for the cache that must include `name="transactions"` in its XML definition.

The XML `name` attribute in each cache configuration must match the required {prodname} cache identifier exactly.
Aside from the cache name, depending on your performance requirements, you can either configure all caches to use the same configuration, or adjust the configuration of each cache independently.
====
...
[[oracle-event-buffering-ehcache]]
==== Ehcache
You can configure a {prodname} Oracle connector to use Ehcache as its cache provider so that the connector buffers event data in a locally embedded cache store.
To enable the connector to use Ehcache, set the value of the `log.mining.buffer.type` to `ehcache`.

You can customize the Ehcache configuration through a set of cache configuration properties.
For details about the available properties, see the xref:oracle-connector-properties[configuration properties] in the `log.mining.buffer.ehcache` namespace.

Some properties apply globally to all Ehcache instances associated with the connector, for example, you might specify a common location for storing all cache data.
Alongside the global configuration, you can set some properties to apply only to specific cache instances, for example, you might set a limit on the number of entries that the cache holds in memory, or specify the maximum size of the persistence store.
The following example shows a possible Ehcache configuration:
...
Additionally, the following example shows some settings that you might configure for a specific embedded cache:

[source,xml]
----
<resources>
    <heap unit="entries">512</heap>
    <disk unit="B">1024000000</disk>
</resources>
----
The following example shows an Ehcache configuration that includes both a global setting and specific settings for the `transactions`, `processedtransactions`, `schemachanges`, `events`, and `rollbacks` caches.
...
----
{
  ...
  "log.mining.buffer.ehcache.transactions.config": "<resources><heap unit=\"entries\">512</entries><disk unit=\"B\">1024000000</disk></resources>",
  "log.mining.buffer.ehcache.processedtransactions.config": "<resources><heap unit=\"entries\">512</entries><disk unit=\"B\">1024000000</disk></resources>",
  "log.mining.buffer.ehcache.schemachanges.config": "<resources><heap unit=\"entries\">512</entries><disk unit=\"B\">1024000000</disk></resources>",
  "log.mining.buffer.ehcache.events.config": "<resources><heap unit=\"entries\">512</entries><disk unit=\"B\">1024000000</disk></resources>",
...
[WARNING]
====
The connector can capture both DDL and DML changes that result from user-initiated or application-level operations.
It can also capture DML changes that originate from `SYS` or `SYSTEM` accounts.
====
...
[horizontal]
`scn`:: The unique identifier that the database uses to track the transaction.
`start_scn`:: The SCN when the transaction started.
...
Please file a {issues-url}[GitHub issue] for any specific types that might be missing.
endif::community[]
...
.RawToStringConverter configuration properties
[cols="1,2,7",options="header"]
|===
...
|`selector`
|_n/a_
|A regular expression that specifies the tables or columns that the converter processes.
If you omit the `selector` property, the converter maps all `RAW` column types to logical `STRING` field types.

|`charset`
|`UTF-8`
|Specifies the character encoding to use when decoding the Oracle raw bytes to a Java string type.
For example, if the database encoding is `WE8ISO8859P1`, this field would need to be set as `ISO-8859-1`, the Java equivalent for the Oracle encoding.
...
----
ifdef::community[]
Oracle AWS RDS does not allow you to execute the commands above nor does it allow you to log in as sysdba.
AWS provides these alternative commands to configure LogMiner.
Before executing these commands, ensure that your Oracle AWS RDS instance is enabled for backups.

To confirm that Oracle has backups enabled, execute the command below first.
The LOG_MODE should say ARCHIVELOG.
If it does not, you may need to reboot your Oracle AWS RDS instance.
...
[TIP]
====
When using the {prodname} Oracle connector in an environment where the standby uses a different archive destination name than the primary instance, the `archive.destination.name` property can include a comma-separated list of destination names.
The values should be ordered by preference as the {prodname} connector will use the destinations to collect logs, and the order to define the log precedence.
This is useful to minimize the reconfiguration necessary when the standby becomes the new primary during a failover scenario.

For example, if the destination for the primary is `LOG_ARCHIVE_DEST_2` and the destination for the standby is `LOG_ARCHIVE_DEST_5`, set the `archive.destination.name` property to `LOG_ARCHIVE_DEST_2,LOG_ARCHIVE_DEST_5`.
When a failover occurs and the standby becomes the primary, Debezium will prioritize destination #2, falling back to destination #5 to pickup any logs that existed prior to the standby being promoted.

If the archive destination is the same for both the primary and standby instances, using a single destination name value is sufficient.
In fact, the property is only required if either instance has more than one archive destination that is both `LOCAL` and `VALID`.
====
...
Typically, when you create the Oracle account for the connector, you grant the account a level of access that permits the connector to detect changes from all tables in the database.
However, in some environments, security policies might prohibit you from granting such a broad level of access.

// I removed the admonition that duplicates the one in the Data change events topic on L1171
...
The grant settings in the example permit the {prodname} user to access all user tables in the database.

To comply with security policies, you can modify the `SELECT ANY TABLE` and `FLASHBACK ANY TABLE` grants so that the connector can access only those tables that you intend to capture.

Do not modify other grants, such as the
ifdef::community[]
`SELECT ANY TRANSACTION` grant, or the
endif::community[]
set of `SELECT ON V_$` grants, which provide access to dynamic performance views (V$).
These grants are required for the connector to function.

[IMPORTANT]
====
To prevent data loss, if you restrict the scope of the `SELECT` and `FLASHBACK` grants, be sure that the modified scope is compatible with the settings in the connector's `include` configuration.
...
|Enables the snapshot process to perform a Flashback snapshot query against any transaction so that the connector can read past changes from LogMiner.
When `FLASHBACK ANY TABLE` is granted, this should also be granted.
ifdef::product[]
Setting this grant is optional.
The connector obtains the required privileges through the `EXECUTE_CATALOG_ROLE` and `LOGMINING` grants.
endif::product[]
ifdef::community[]
This grant is optional for Oracle 12c and later.

In those later releases, the connector obtains the required privileges through the `EXECUTE_CATALOG_ROLE` and `LOGMINING` grants.
endif::community[]
|9
|LOGMINING
...
The {prodname} Oracle connector can capture change events from a physical standby database by connecting to the primary for snapshots and metadata queries while streaming changes from the standby.
You can xref:configuring-the-debezium-oracle-connector-to-capture-events-from-a-physical-standby[configure the {prodname} Oracle connector to capture changes from a physical standby database].
...
// Type: procedure
[id="configuring-the-debezium-oracle-connector-to-capture-events-from-a-physical-standby"]
==== Configuring the {prodname} Oracle connector to capture events from a physical standby

ifdef::community[]
[NOTE]
====
The ability to use the connector with a physical standby is in an incubating state and can change without notice.
====
endif::community[]

The {prodname} Oracle connector can stream changes directly from a physical standby database while using the primary for snapshots and metadata queries.
This reduces the load on the primary by offloading LogMiner operations to the standby.

.Prerequisites

* The physical standby database is configured with managed recovery and is synchronized with the primary instance.
* The standby database is open in read-only mode (Active Data Guard).
* A flat file data dictionary has been exported from the primary instance and is accessible from the standby database.
* The `log.mining.strategy` is set to `dictionary_from_file`.

.Procedure

* Add the following properties to the connector configuration:
+
```
log.mining.strategy=dictionary_from_file
log.mining.path.dictionary=/path/to/dictionary.ora
log.mining.read.only=true
secondary.hostname=standby-host
```
+
Alternatively, you can use `secondary.url` to specify the full JDBC connection string to the standby:
+
```
secondary.url=jdbc:oracle:thin:@standby-host:1521/ORCLPDB1
```
+
If the standby uses a different port than the primary, you can also set `secondary.port`.
If the standby uses a different database name than the primary, you can also set `secondary.dbname`.

When configured with these properties, the connector:

1. Connects to the primary database (configured via `database.hostname`) for snapshot operations and metadata queries.
2. Connects to the standby database (configured via `secondary.hostname` or `secondary.url`) in read-only mode for LogMiner streaming operations.
3. Uses the flat file data dictionary to resolve table and column names during log mining on the standby.
...
[IMPORTANT]
====
The ability for the {prodname} Oracle connector to ingest changes from a read-only logical standby database is a Technology Preview feature.
...
[WARNING]
====
The ability to use the connector with a logical standby is in an incubating state and can change without notice.
====
endif::community[]
...
You can use the `log.mining.read.only` property with an Oracle Standalone database, or with an Oracle RAC installation.
...
// Title: Using Java key and trust stores to configure the Oracle connector to use mTLS
[[using-mtls-secure-connections-java-key-trust-stores]]
...
.Procedure
* Configure the {prodname} Oracle connector, as illustrated in the following example.
+
.Example: {prodname} Oracle connector TLS configuration
[source,json,indent=0]
----
{
...
[NOTE]
====
For {prodname} to use TLS encryption to communicate with Oracle, requires a TCPS connection with the server.
To establish a TCPS connection, you must configure the connector to use the `database.url` property, and not the `database.host` property.
Unlike the `database.host` property, the `database.url` property permits you to define an Oracle TNS (Transparent Network Substrate) connection string that explicitly requires use of the `TCPS` protocol.
====

// Type: procedure
// ModuleID: oracle-connector-using-oracle-wallet-to-configure-mtls
// Title: Using Oracle wallet to configure the connector to use mTLS
[[using-mtls-secure-connections-oracle-wallet]]
==== Oracle Wallet
...
`logminer` (default):: The connector uses the native Oracle LogMiner API with connector-level buffering.
`logminer_unbuffered`:: The connector uses the native Oracle LogMiner API with database-level buffering.
ifdef::community[]
...
For example,
+
`+"snapshot.select.statement.overrides": "ORCLPDB1.inventory.products,ORCLPDB1.customers.orders"+`
+
...
+
For example, to specify a SELECT statement for the `ORCLPDB1.customer.orders` table, add the following property:
+
`snapshot.select.statement.overrides.ORCLPDB1.customers.orders`
+
[WARNING]
====
...
+
Example configuration::
The following example shows how to configure the `snapshot.select.statement.overrides` property to perform a snapshot of the `ORCLPDB1.customers.orders` table that includes only records that are not soft-deleted; that is, the value of the soft-delete field, `delete_flag`, is set to `0`.
====
----
"snapshot.select.statement.overrides": "ORCLPDB1.customer.orders",
"snapshot.select.statement.overrides.ORCLPDB1.customer.orders": "SELECT * FROM customers.orders WHERE delete_flag = 0 ORDER BY id DESC"
----
====
...
For more information, see xref:oracle-streaming-metrics[Streaming metrics].
...
ifdef::community[]
|[[oracle-property-log-mining-read-only]]<<oracle-property-log-mining-read-only, `+log.mining.read.only+`>>
|`false`
|When set to `true`, the connector does not attempt to flush the LGWR buffer to disk, allowing it to connect to read-only databases such as logical standbys or physical standbys.

|[[oracle-property-log-mining-path-dictionary]]<<oracle-property-log-mining-path-dictionary, `+log.mining.path.dictionary+`>>
|No default
|Specifies the absolute file path on the database server where Oracle LogMiner can access the flat file data dictionary.
Required when `log.mining.strategy` is set to `dictionary_from_file`.

|[[oracle-property-capture-mode]]<<oracle-property-capture-mode, `+capture.mode+`>>
|`primary`
|Specifies the streaming capture mode to be used.
...
|No default
|Specifies the hostname of the secondary database from which the connector streams changes.
When set, the connector uses the primary database (configured via `database.hostname`) for snapshots and metadata queries, and the secondary for streaming.

|[[oracle-property-secondary-port]]<<oracle-property-secondary-port, `+secondary.port+`>>
|Same as `database.port`
|Specifies the port of the secondary database used for streaming, if it differs from the primary instance port.

|[[oracle-property-secondary-dbname]]<<oracle-property-secondary-dbname, `+secondary.dbname+`>>
|No default
|Specifies the database name of the secondary database used for streaming.

|[[oracle-property-secondary-url]]<<oracle-property-secondary-url, `+secondary.url+`>>
|No default
|Specifies the full JDBC connection string to the secondary database used for streaming.
Use this as an alternative to `secondary.hostname`, `secondary.port`, and `secondary.dbname` when the connection string requires additional parameters.
endif::community[]
...
This strategy is deprecated and scheduled for removal in a future {prodname} release.
Consider using the `hybrid` strategy instead.
...
ifdef::community[]
`dictionary_from_file`:: The connector uses the dictionary from a flat file located on the database server.
This strategy is required when streaming changes from a physical standby database.
endif::community[]
...
|`true`
|Specifies whether the rollback segment identifier (`RS_ID`) values are provided in change events.
...
|[[oracle-property-log-mining-buffer-infinispan-cache-rollbacks]]<<oracle-property-log-mining-buffer-infinispan-cache-rollbacks, `+log.mining.buffer.infinispan.cache.rollbacks+`>>
|No default
|The XML configuration for the Infinispan rollback events cache.
For more information, see xref:oracle-event-buffering-infinispan[Infinispan event buffering].
...
|[[oracle-property-log-mining-buffer-ehcache-global-config]]<<oracle-property-log-mining-buffer-ehcache-global-config, `+log.mining.buffer.ehcache.global.config+`>>
|No default
|The XML configuration for the Ehcache global configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].

|[[oracle-property-log-mining-buffer-ehcache-transactions-config]]<<oracle-property-log-mining-buffer-ehcache-transactions-config, `+log.mining.buffer.ehcache.transactions.config+`>>
|No default
|The XML configuration for the Ehcache transactions configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].

|[[oracle-property-log-mining-buffer-ehcache-processedtransactions-config]]<<oracle-property-log-mining-buffer-ehcache-processedtransactions-config, `+log.mining.buffer.ehcache.processedtransactions.config+`>>
|No default
|The XML configuration for the Ehcache processed/emitted transactions configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].

|[[oracle-property-log-mining-buffer-ehcache-schemachanges-config]]<<oracle-property-log-mining-buffer-ehcache-schemachanges-config, `+log.mining.buffer.ehcache.schemachanges.config+`>>
|No default
|The XML configuration for the Ehcache schema changes configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].

|[[oracle-property-log-mining-buffer-ehcache-events-config]]<<oracle-property-log-mining-buffer-ehcache-events-config, `+log.mining.buffer.ehcache.events.config+`>>
|No default
|The XML configuration for the Ehcache events configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].

|[[oracle-property-log-mining-buffer-ehcache-rollbacks-config]]<<oracle-property-log-mining-buffer-ehcache-rollbacks-config, `+log.mining.buffer.ehcache.rollbacks.config+`>>
|No default
|The XML configuration for the Ehcache rollback events configuration.
For more information, see xref:oracle-event-buffering-ehcache[Ehcache event buffering].
...
|[[oracle-property-log-mining-buffer-deferred-transaction-cache-pin]]<<oracle-property-log-mining-buffer-deferred-transaction-cache-pin, `log.mining.buffer.deferred.transaction.cache.pin`>>
|`false`
|Boolean value that specifies whether the mining window is pinned to the oldest promoted (DML-bearing) transaction in the cache when deferred transaction start behavior is enabled.
When set to `true`, the mining window is pinned to the oldest cached transaction's start SCN, similar to the non-deferred path, but only for transactions that have emitted DML.
When set to `false` (the default), the mining window slides block-by-block and is not pinned by transactions in the cache.
This option has no effect when `log.mining.buffer.deferred.transaction.start` is not enabled.
...
|`0`
|Positive integer value that specifies the maximum duration in milliseconds that the LogMiner mining window lower bound can remain at the same position due to a long-running transaction.
When set to `0` (default), the feature is disabled and the lower bound is always set to the oldest transaction's start SCN.

When a transaction exceeds this threshold, the mining session lower bound advanceds to the start SCN of the next oldest transaction within the threshold.

This feature is useful in environments with long-running transactions, particularly when those transactions are on tables not included in the connector's table filters.

By advancing the session bounds, the connector can reduce the mining session window size, improving query performance without abandoning the transaction.

[WARNING]
====
Setting this value may increase the risk of edge cases involving missed commits or unsupported transactions.

Carefully evaluate the risk of these edge cases occurring in your environment relative to the performance benefits of preventing mining window growth.
====
endif::community[]
...
|[[oracle-property-log-mining-clientid-include-list]]<<oracle-property-log-mining-clientid-include-list, `+log.mining.clientid.include.list+`>>
|No default
|List of JDBC connection client ids to include from the LogMiner query.
It can be useful to set this property if you want the capturing process to include only changes from the specific JDBC connection clients.

|[[oracle-property-log-mining-clientid-exclude-list]]<<oracle-property-log-mining-clientid-exclude-list, `+log.mining.clientid.exclude.list+`>>
|No default
...
|[[oracle-property-custom-sanitize-pattern]]<<oracle-property-custom-sanitize-pattern, `custom.sanitize.pattern`>>
...
This limit applies not only to tables being captured for data changes but also to any tables whose schemas are tracked as part of schema history.
...
|[[oracle-property-guardrail-collections-limit-action]]<<oracle-property-guardrail-collections-limit-action, `+guardrail.collections.limit.action+`>>
|`warn`
|Specifies the action to trigger if the number of tables that the connector captures exceeds the number that you specify in the `+guardrail.collections.max+` property.
Set the property to one of the following values:

`fail`::
The connector fails and reports an exception.

`warn`::
The connector logs a warning.

|[[oracle-property-extended-headers-enabled]]<<oracle-property-extended-headers-enabled, `extended.headers.enabled`>>
|true
...
The {prodname} Oracle connector provides three metric types in addition to the built-in support for JMX metrics that Apache Kafka and Kafka Connect have.
...
|[[oracle-streaming-metrics-mining-session-lower-bounds]]<<oracle-streaming-metrics-mining-session-lower-bounds, `+MiningSessionLowerBounds+`>>
|`BigInteger`
|The system change number currently used as the LogMiner session lower boundary.

|[[oracle-streaming-metrics-mining-session-upper-bounds]]<<oracle-streaming-metrics-mining-session-upper-bounds, `+MiningSessionUpperBounds+`>>
|`BigInteger`
|The system change number currently used as the LogMiner session upper boundary.

|[[oracle-streaming-metrics-mining-fetch-lower-bounds]]<<oracle-streaming-metrics-mining-fetch-lower-bounds, `+MiningFetchLowerBounds+`>>
|`BigInteger`
|The system change number currently used as the LogMiner fetch query lower boundary.

|[[oracle-streaming-metrics-mining-fetch-upper-bounds]]<<oracle-streaming-metrics-mining-fetch-upper-bounds, `+MiningFetchUpperBounds+`>>
|`BigInteger`
...
|[[oracle-streaming-metrics-maxcaptureddmlcountinbatch]]<<oracle-streaming-metrics-maxcaptureddmlcountinbatch, `+MaxCapturedDmlCountInBatch+`>>
|`long`
...
|[[oracle-streaming-metrics-totalcommittimeinmilliseconds]]<<oracle-streaming-metrics-totalcommittimeinmilliseconds, `+TotalCommitTimeInMilliseconds+`>>
|`long`
|The total duration in milliseconds spent emitting a transaction after a commit is observed.
...
|[[oracle-streaming-metrics-number-of-events-in-buffer]]<<oracle-streaming-metrics-number-of-events-in-buffer, `+NumberOfEventsInBuffer+`>>
|`long`
|The number of events in the transaction buffer.
...
|`long`
|The number of emitted events per second during transaction commit windows.
...
|[[oracle-streaming-metrics-total-schema-change-parse-error-count]]<<oracle-streaming-metrics-total-schema-change-parse-error-count, `+TotalSchemaChangeParseErrorCount+`>>
|`int`
...
The following example illustrates how to configure this on a specific table, which is the ideal choice to minimize the amount of information captured in the Oracle redo logs.
...
ifdef::community[]
(given the right privileges, this might be done automatically by the connector going forward, see https://issues.redhat.com/browse//browse/DBZ-721[DBZ-721]):
endif::community[]
...
The following configuration example adds the properties `database.connection.adapter` and `xstream.out.server.name` to enable the connector to use the XStream API implementation.
...
|[[oracle-property-xstream-out-server-name]]<<oracle-property-xstream-out-server-name, `+xstream.out.server.name+`>>
|No default
...
Note that this strategy is now deprecated; the `hybrid` strategy provides equivalent reliability with better performance.
+
...
[id="what-causes-redo-thread-inconsistency"]
*What causes redo thread inconsistency?*::
The {prodname} Oracle connector can capture changes only when all required database transaction logs are available and are free of sequence gaps.
When the connector uses the LogMiner adapter, and this requirement is not met, log entries might report that an Oracle redo thread is inconsistent, for example:
+
```
...
+
--
A variety of factors can trigger the preceding inconsistent redo thread error.
The following list highlights some possible contributing factors:

* During shutdown of an Oracle RAC node, the redo thread transitions from OPEN to CLOSED.
* An online redo log reaches maximum capacity and is queued for archiving, but the archive process cannot complete (for instance, because the destination is full or unavailable).
* The resume read position for the connector is not available.
--
+
The most common reason for this log entry is that an online redo log the connector requires has been queued for archival.
When a log is scheduled for archival, a database process called ARC copies the online redo log on disk to the archive destination(s).
When an Oracle system is under load or there are several queued archival jobs, it could take a short period of time before the online redo log is registered in `V$ARCHIVED_LOG` by the ARC process when the archival completes.
Because {prodname} does not have direct file system access, the log must be registered in the performance view before {prodname} can continue.
+
In some instances, redo thread inconsistency results in an error that reports the SCN is not available.
If this exception occurs, check the log entries that precede the error for indications that connector exceeded the maximum number of retries.
+
When the Oracle ARC process takes more time to complete the archival than the {prodname} Oracle connector allows, you can adjust the internal connector configuration property `internal.log.mining.query.max.retries` to increase the number of retry attempts.
By default, the connector retries up to `5` attempts before it throws an exception.
...
```
+
If an ORA-25191 exception is thrown, please raise a GitHub issue with the details about the table and it's mappings, related to other parent tables, etc.
...
*What causes ORA-01013 and how can I resolve it?*::
When a query runs for too long, the {prodname} Oracle connector might report the following exception:
+
```
ORA-01013: user requested cancel of current operation
```
+
The {prodname} Oracle connector provides a configuration option, xref:oracle-property-database-query-timeout-ms[`database.query.timeout.ms`], which specifies how long a query can execute before it is canceled.
Configuring this timeout prevents the connector from allocating resources to queries for too long, which can degrade database performance.
In most circumstances, the default of ten minutes provides a reasonable execution time while guarding against excessive database resource consumption.
+
You can increase the default value to provide a higher threshold before queries are canceled, particularly if the exception occurs frequently.
You can also set the value to `0` to disable query timeouts and permit all queries to run until they return results.
...
*What's the cause for ORA-00600 and how to handle it?*::
ORA-00600 is a generic internal error. It indicates the relevant process has encountered a low-level unexpected condition - which typically means you have encountered a bug. See this https://blogs.oracle.com/database/post/ora-00600[Oracle Blog] for more information.
+
ORA-00600 comes along with multiple arguments whereas the first one is the most significant. Depending on the first argument there is a supposed solution to be applied:
+
```
ORA-00600: internal error code, arguments: [flg0], ...
```
+
The solution to avoid this issue is to enable supplemental logging on db level.
+
```
ORA-00600: internal error code, arguments: [kdli_iread], ...
```
+
This exception can be avoided by upgrading the Oracle db to 19.25 and applying patch 32209850.
...
The OCI native library `libocijdbcXX.so` has a runtime dependency on `libaio`.
...
.. For RHEL/UBI/CentOS, run the following:
+
[source,bash]
----
yum install -y libaio
----
.. For Debezium/Ubuntu, run the following:
+
[source,bash]
----
