...
[IMPORTANT]
====
The Committed changes mode setting is a Technology Preview feature.
...
`internal.log.mining.read.only`:: This property is set to `true` to enable capturing changes from a downstream mining database, rather than from the primary instance.
`log.mining.read.only.hostname`:: Specifies the hostname of the downstream mining database from which you want the {prodname} Oracle connector to read and capture transaction logs.
...
ifdef::community[]
|[[oracle-property-log-mining-readonly-hostname]]<<oracle-property-log-mining-readonly-hostname, `+log.mining.readonly.hostname+`>>
|`null`
|Specifies the hostname of the downstream read-only mining database from which you want the {prodname} Oracle connector to capture transaction logs.
endif::community[]
...
ifdef::community[]
`dictionary_from_file`:: The connector uses the dictionary from a file located in an Oracle directory object.
For more information, see xref:oracle-logminer-mining-database[Capturing changes from downstream mining databases].
endif::community[]
...
|[[oracle-property-log-mining-session-max-transaction-age-ms]]<<oracle-property-log-mining-session-max-transaction-age-ms, `log.mining.session.max.transaction.age.ms`>>
|`0`
|Positive integer value that specifies the maximum age, in milliseconds, that a transaction can remain pending before the mining session lower bound advances past it.
When set to `0` (default), the feature is disabled and the lower bound is always set to the oldest transaction's start SCN.

The age is measured as the duration between the transaction's start time and the current mining position.
When a transaction exceeds this threshold, the mining session and query lower bounds advance to the start SCN of the next oldest transaction within the threshold.
Long-running transactions that exceed the age threshold remain tracked in the transaction buffer and will still be committed when their commit record is encountered.
...
By allowing the session bounds to advance, the connector can reduce the size of the mining session window, improving query performance without abandoning the transaction.
Setting this value may increase the risk of edge cases involving missed commits/rollbacks or unsupported transactions.
