 ...
 * <ul>
 *   <li><b>WORKER</b> units -- one per log (or one per group of logs if N &lt; log count).
 *       Workers run in parallel on independent log slices.</li>
 *   <li><b>BRIDGE</b> units -- created after workers complete, for orphan commits that have a
 ...
 * <ul>
 *   <li>{@link #pendingInheritedTransactions} -- open transactions carried from prior waves</li>
 *   <li>{@link #pendingOrphanCommits} -- orphan commits waiting to be matched with an inherited tx</li>
 * </ul>
 *
 ...
 * <ol>
 *   <li>{@code log.mining.concurrent.readers > 1} -- must be configured; checked in
 *       {@link io.debezium.connector.oracle.logminer.buffered.BufferedLogMinerAdapter}.</li>
 *   <li>Only archive logs in scope -- if any online redo log is in the current wave's log set, fall
 *       back to single-threaded mode for that wave.</li>
 *   <li>At least 2 archive logs in scope -- if only 1, fall back to single-threaded mode.</li>
 * </ol>
 *
...
public class LogMinerWorkerCoordinator implements AutoCloseable {
     ...
     */
    private final List<LogMinerSchemaChangeRecord> pendingSchemaChanges = new ArrayList<>();
    ...
    *       BRIDGE units.</li>
    ...
    public Scn executeWave(
        ...
        final List<LogMinerSchemaChangeRecord> savedPendingSchemaChanges = new ArrayList<>(pendingSchemaChanges);
        ...
        final List<LogMinerSchemaChangeRecord> appliedWaveSchemaChanges = new ArrayList<>();
        try {
    ...
    /**
     * Runs a <strong>serial</strong> coordination wave -- exactly one worker processes all logs.
     ...
     * calling thread -- it does not use the thread pool. This guarantees that online redo
