...
public class LogMinerEventRow {
    ...
    private void initializeFromResultSet(ResultSet resultSet, OracleDatabaseSchema schema,
        ...
        // Positions 10+: pre-computed by LogMinerColumnIndexes to account for omitted optional columns.
        // Optional columns have a null index when disabled; mandatory columns always have a non-null index.
        if (indexes.getUsernameIndex() != null) {
            this.userName = resultSet.getString(indexes.getUsernameIndex());
        }
        this.rowId = resultSet.getString(indexes.getRowIdIndex());
        this.rollbackFlag = resultSet.getInt(indexes.getRollbackFlagIndex()) == 1;
        if (indexes.getRsIdIndex() != null) {
            this.rsId = trim(resultSet.getString(indexes.getRsIdIndex()));
        }
        this.redoSql = getSqlRedo(resultSet);
        this.status = resultSet.getInt(indexes.getStatusIndex());
        this.info = resultSet.getString(indexes.getInfoIndex());
        this.ssn = resultSet.getLong(indexes.getSsnIndex());
        this.thread = resultSet.getInt(indexes.getThreadIndex());
        this.objectId = resultSet.getLong(indexes.getObjectIdIndex());
        this.objectVersion = resultSet.getLong(indexes.getObjectVersionIndex());
        this.dataObjectId = resultSet.getLong(indexes.getDataObjectIdIndex());
        if (indexes.getClientIdIndex() != null) {
            this.clientId = resultSet.getString(indexes.getClientIdIndex());
        }
        this.startScn = getScn(resultSet, indexes.getStartScnIndex());
        this.commitScn = getScn(resultSet, indexes.getCommitScnIndex());
        if (indexes.getStartTimestampIndex() != null) {
            this.startTime = getTime(resultSet, indexes.getStartTimestampIndex());
        }
        if (indexes.getCommitTimestampIndex() != null) {
            this.commitTime = getTime(resultSet, indexes.getCommitTimestampIndex());
        }
        this.transactionSequence = resultSet.getLong(indexes.getSequenceIndex());
