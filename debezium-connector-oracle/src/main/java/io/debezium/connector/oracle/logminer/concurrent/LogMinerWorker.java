 ...
 *   <li>orphan commits -- COMMITs whose STARTs were not in either the inherited list or the inherited list or the
 *       read window</li>
...
public class LogMinerWorker implements Callable<WorkerResult> {
    ...
    public interface SchemaChangeListener {
        void onSchemaChange(LogMinerSchemaChangeRecord change) throws InterruptedException;
    }
    ...
    public WorkerResult call() throws Exception {
        ...
        // Any transaction still in the cache is unresolved -- carry forward
    ...
    private void processRow(LogMinerEventRow row,
        ...
        if (!handlePayloadEvent(type, row, cacheProvider, transactionFactory, dmlParser, schemaChanges)) {
            // Skip LOB, XML, unsupported etc. -- workers produce raw events only
    ...
    private boolean handleTransactionControlEvent(EventType type,
        ...
        switch (type) {
            ...
            case ROLLBACK -> {
                ...
                LogMinerTransactionCacheHelper.removeTransactionAndEvents(cacheProvider.getTransactionCache(), transactionId);

                if (LogMinerTransactionCacheHelper.isPartialTransactionId(transactionId)) {
                    final String prefix = LogMinerTransactionCacheHelper.getPartialTransactionPrefix(transactionId);
                    LOGGER.debug("Worker {} rollback for partial transaction '{}' with prefix '{}'",
                            workerId, transactionId, prefix);
                    final List<MemoryTransaction> matchingTransactions = LogMinerTransactionCacheHelper.findTransactionsByPrefix(
                            cacheProvider.getTransactionCache(), prefix);
                    if (matchingTransactions.size() == 1) {
                        final MemoryTransaction matched = matchingTransactions.get(0);
                        LOGGER.warn("Worker {} matched partial transaction '{}' to cached transaction '{}' for rollback",
                                workerId, transactionId, matched.getTransactionId());
                        LogMinerTransactionCacheHelper.removeTransactionAndEvents(cacheProvider.getTransactionCache(), matched.getTransactionId());
                    }
                    else if (matchingTransactions.size() > 1) {
                        LOGGER.warn("Worker {} unable to match partial transaction '{}' to single cached transaction; found {} matches with prefix '{}'",
                                workerId, transactionId, matchingTransactions.size(), prefix);
                    }
                }
                return true;
            }
    ...
    private void handleDdl(LogMinerEventRow row,
        ...
        if (LogMinerEventDispatchHelper.isSchemaChangeEventSkipped(connectorConfig, schema, null, row)) {
            return;
        }
    ...
    private void emitSchemaChange(LogMinerSchemaChangeRecord schemaChange,
                                  List<LogMinerSchemaChangeRecord> schemaChanges)
            throws InterruptedException {
    ...
    private void handleDml(LogMinerEventRow row,
        ...
        if (!LogMinerEventDispatchHelper.isDmlEventEligible(row)) {
            LOGGER.trace("Worker {} skipping ineligible DML event at SCN {}", workerId, row.getScn());
            return;
        }
    ...
    private void handleCommit(LogMinerEventRow row,
        ...
        if (tx == null) {
            // We have no record of this transaction's START -- it is an orphan commit
